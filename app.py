...
🚫 BANNED:
- NO "Sage", "Wisdom", "Ancient", "Heroes". 
- NO POETRY.
- NO DIRECT INSULTS ("Ugly", "Fat").

✅ REQUIRED VIBE:
- Roast the user's CURRENT ROUTINE (e.g., if they use soap, destroy them).
- Use specific modern scenarios (Crypto, Zoom, Dating Apps).
...
# UPDATED FUNCTION WITH NAME AND ROUTINE
def analyze_skin_with_vision(image_file, age, skin_type, problem, habits, routine, name):
...
def create_pdf(data, problem_name, user_name):
    """Generate PDF"""
    pdf = FPDF()
    ...
    # PERSONALIZED TITLE
    title = f"{clean_text(user_name).upper()}'S UPGRADE PLAN" if user_name else "YOUR UPGRADE PLAN"
...
if st.query_params.get("paid") == "true":
    ...
    p_name = st.text_input("First Name (So I can roast you personally):")
    p_email = st.text_input("Email (Where to send backup):")
    
    st.write("### 2. Skin Profile:")
    ...
    # NEW FIELD: ROUTINE
    p_routine = st.selectbox("Current Routine (Be Honest)", 
                             ["I use 3-in-1 Body Wash/Soap", 
                              "I splash water and pray", 
                              "I use a random moisturizer", 
                              "I steal my girlfriend's products",
                              "I actually have a routine"], key="p_routine")
    ...
    if st.button("GENERATE REPORT NOW"):
        if upl and p_name:
            with st.spinner("SCANNING FACE & DATA..."):
                # Pass Name and Routine to AI
                data = analyze_skin_with_vision(upl, p_age, p_skin, p_enemy, p_sins, p_routine, p_name)
                if data:
                    try:
                        pdf = create_pdf(data, p_enemy, p_name)
                        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                            pdf.output(tmp.name)
                            with open(tmp.name, "rb") as f:
                                st.download_button("⬇️ DOWNLOAD FULL DOSSIER (PDF)", f, "Skin_Roast_Personal.pdf", "application/pdf")
