...
def clean_text(text):
    """
    Универсальный чистильщик текста для PDF.
    Заменяет юникод-символы на эквиваленты latin-1, 
    чтобы PDF-генератор не вылетал с ошибкой.
    """
    if isinstance(text, str):
        # Словарь замен для символов, которые ломают latin-1
        replacements = {
            ...
            '\u2014': '-',   # Длинное тире
            '\u2026': '...', # Многоточие
            '’': "'",        # Обычный типографский апостроф
            '—': '-',        # Русское/длинное тире
            '“': '"',
            '”': '"'
        }
        ...
        # Ключевой момент: кодируем в latin-1, заменяя все НЕИЗВЕСТНОЕ (в т.ч. кириллицу) 
        # на знак вопроса или пробел, чтобы программа НЕ падала.
